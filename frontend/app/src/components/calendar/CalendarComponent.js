...
import { Table } from "react-bootstrap";
  ...
  export default function CalendarComponent() {
    ...
    const fetchAbsentEmployeesInfo = async() => {
        const absentEmpPerDay = await fetch(API_URL + "absence", {
          method: "GET",
    ...
    function convertAbsentEmployeesInfo(){
        ...
        for(var x = 0; x < absentEmployeesData.length; x++){
            ...
            month = splitted[1] - 1;
            day =  splitted[2];
            convertedToCorrectStructure.push([new Date(year, month, day), absentEmployeesData[x]["absentEmployees"].length]);
        }
    ...
    function generateEmployeeTable(employeeList, date){     
        ...
        if(employeeList.length === 0){
            ...
            return(
                ...
                <tbody>{employeeList.map((employee) => {
                    id ++;
                    return(
                        <tr key={id}><td>{employee.split(" ")[0]}</td><td>{employee.split(" ")[1]}</td></tr>
                    )
                })}</tbody>
                </Table>
            </div>
    ...
    absentEmployeesData.forEach((v) => absentEmployeesMap.set(v["day"], v["absentEmployees"]));
    const data = [
        [
          {
            type: "date",
    ...
    return(
        <div className="ml-3">
            ...
            colorAxis:{minValue: 0,  colors: ['#ADD8E6', '#00008b']},
            calendar: {
                cellColor: {
                stroke: '#CCCCCC',
                strokeOpacity: 0.5,
                ...
                google.visualization.events.addListener(chart, "select", e => {
                    ...
                    if(selection !== undefined && 'row' in selection){
                        ...
                        if(currentViewedDay == checkedDay){
                            ...
                            setcurrentEmployeesShown(generateEmployeeTable(employeesList, checkedDay));
                        }
                    }
                });
              }
